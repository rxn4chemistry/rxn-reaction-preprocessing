...
from rxn_reaction_preprocessing.config import StandardizeConfig, PreprocessConfig, FragmentBond
from rxn_reaction_preprocessing.mixed_reaction_filter import ReactionFilterError
from rxn_reaction_preprocessing.molecule_standardizer import MoleculeStandardizer, RejectedMolecule
            ...
            [
                {
                    "type":
                        "Add",
                    "text":
                        "BuLi (100 ml, 250 mmol, 2.5 M in hexanes) was added dropwise",
                    "components":
                        [
                            {
                                "role":
...
def test_pistachio_record_standardizer():
    ...
    updated_record = standardizer.standardize(record)
    assert updated_record == create_dummy_record('CC.O.[Na]Cl>>CCO', '[Na]Cl', 'O', 'CC')

    # Test 3:
    # same as test 2, with the input in extended SMILES format, and atom mapping
    record = create_dummy_record(
        '[Na+].[Cl-].[CH3:1][CH3:2].[OH2:3]>>[CH3:1][CH2:2][OH:3] |f:0.1|', '[Na+].[Cl-]', 'O',
        'CC'
    )
    updated_record = standardizer.standardize(record)
    assert updated_record == create_dummy_record('CC.O.[Na]Cl>>CCO', '[Na]Cl', 'O', 'CC')

...
def duplicate_reaction():
    return Reaction('[14C]Cl.[14C]Cl.[Na]O>O.O>[Na]Cl.[Na]Cl.[14C]O', remove_duplicates=True)
...
def test_sort_only_reactants(reaction):
    assert (
        str(reaction.sort(sort_products=False,
...
def test_remove_precursors_from_products(dirty_reaction):
    assert (
        str(dirty_reaction.remove_precursors_from_products()) == '[14C]Cl.O[Na]>O>[Na]Cl.[14C]O'
    )

...
def test_missing_annotations_in_reaction_equations():
    ...
    expected_molecules = [
        expected_molecule for _, expected_list in reactions_and_expected_annotations
...
def test_missing_annotations_in_reaction_smiles():
    ...
    # one reactant and product must be annotated
    assert list(mad.missing_in_reaction_smiles('C.C1NC1>O>CCC1CC1')) == ['C1NC1', 'CCC1CC1']
    # one molecule with fragment bond must be annotated
    assert list(mad.missing_in_reaction_smiles('C.N~CCC1NC1>O>CCCC', '~')) == ['N.CCC1NC1']

...
def test_annotated_as_rejected():
    ...
    annotations = [
        MoleculeAnnotation(
            original_smiles=rejected_smiles, updated_smiles=None, decision='reject', categories=[]
        )
    ]
...
def test_missing_annotation():
    ...
    annotations = [
        MoleculeAnnotation(
            original_smiles=tm_smiles, updated_smiles=None, decision='accept', categories=[]
        )
    ]
...
def test_canonicalization_before_rejection_check():
    ...
    _ = MoleculeStandardizer(annotations=annotations, canonicalize=False)(non_canonical_smiles)
...
def test_missing_annotation_check_after_rejection_check():
    ...
    standardizer = MoleculeStandardizer(annotations=annotations, discard_missing_annotations=True)
    with pytest.raises(RejectedMolecule):

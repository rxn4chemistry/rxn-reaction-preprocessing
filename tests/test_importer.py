...
from rxn_reaction_preprocessing.config import RxnImportConfig, InitialDataFormat, FragmentBond
from rxn_reaction_preprocessing.importer import rxn_import, InvalidColumn, InvalidType
...
def test_smiles_format_is_updated(input_file: str, output_file: str):
    ...
    reactions = ['CC>>CC', 'OO>>OO', 'C.C.O>>CCO', 'C.[Na+]~[Cl-]>>C', 'C.[Na+].[Cl-]>>C |f:1.2|']
    expected = ['CC>>CC', 'OO>>OO', 'C.C.O>>CCO', 'C.[Na+]~[Cl-]>>C', 'C.[Na+]~[Cl-]>>C']
...
def test_light_and_heat(input_file: str, output_file: str):
    ...
    expected = [
        'CC>>CC', f'OO.{LIGHT_TOKEN}.{HEAT_TOKEN}>>OO', f'C.C.O.{LIGHT_TOKEN}>>CCO',
        f'C=C.{HEAT_TOKEN}>>CC'
    ]
...
def test_light_and_heat_with_non_boolean_type(input_file: str, output_file: str):
    ...
    pd.DataFrame({'smiles': reactions, 'light': has_light}).to_csv(input_file, index=False)

...
def split_smile(smi):
    ...
    try:
        ...
        splittedsmi = RF.get_fragmented_reaction_smiles(mixed = True, order_molecules = True, fragment_bond = '~')
    except:
        splittedsmi = ' '
    return splittedsmi
...
def passed_filter(x):
    ...
    return rxn_filter.apply_all_filters()
...
class DataExtractionCleaning:
    ...
    def split_class_info(self):
        ...
        self.df['classes_long'] = self.df['classes_long'].apply(lambda x: [l.strip() for l in x.split()])
        self.df['US_classes']=self.df['classes_long'].apply(lambda x: x[0])
        self.df['classes_ids']=self.df['classes_long'].apply(lambda x: x[1].replace('[','').replace(']',''))
        self.df['classes_names']=self.df['classes_long'].apply(lambda x: " ".join(x[2:]))
    ...
    def get_fragmented_rxn_smiles(self):
        ...
        self.df['reactions_mixed'] = [split_smile(rxn) for rxn in tqdm(self.df.reactions)]
    ...
    def filter_dataset(self, filtername):
        ...
        self.df.loc[self.df['filtering'] == False].to_csv(self.destpath + f'/dump_step_2_{fname}_removed.csv')
    ...
    def split_precursors_products(self):
        ...
        self.df['splitted_reactions_mixed'] = [x.split('>>') for x in self.df.reactions_mixed]
        print("Putting the precursors with multiple appearances in 'precursors_multi' ...")
        self.df['precursors_multi'] = [x[0] for x in self.df.splitted_reactions_mixed]
        print("Putting the products with multiple appearances in 'precursors_multi' ...")
    ...
    def tokenize(self):
        ...
        self.df['tokenized_reactions_mixed'] = self.df.tokenized_precursors + " >> " + self.df.tokenized_products
        return
    ...
    def keep_largest_fragment(self):
        ...
        self.df['largest_fragment_products'] = [Chem.MolToSmiles(lfc.choose(Chem.MolFromSmiles(smi.replace('~', '.')))) for smi in tqdm(self.df.products)]

        print('Size od dataset: {}'.format(len(self.df)))
        self.df['tokenized_largest_fragment_products'] = [smi_tokenizer(x) for x in self.df.largest_fragment_products]
        print('Kept largest fragment in product')
        return


    def remove_purification(self):
        self.df['is_in_precursors'] = [is_in_precursors(row) for idx,row in tqdm(self.df.iterrows())]
        print('Size od dataset: {}'.format(len(self.df)))
    ...
    def do_set_on_precursors_and_products(self):
        ...
        self.df['precursors'] = ['.'.join(sorted(list(set(x.split('.'))))) for x in self.df['precursors_multi'].values]
        self.df['products'] = ['.'.join(sorted(list(set(x.split('.'))))) for x in self.df['products_multi'].values]
    ...
    def remove_residual_reagents(self):
        ...
        self.df['products_no_residual'] = [remove_res_reagents(x) for x in tqdm(self.df[['precursors','products']].values)]
    ...
    def remove_single_precursor_rxns(self):
        ...
        self.df['splitted_classes_names'] = [x.split(" ")[-1] for x in self.df.classes_names]
        print(self.df[['splitted_classes_names','classes_names']].loc[0:10])
        self.df.loc[(self.df['splitted_classes_names']==saveset[0]) & (self.df['filtering'] == False),'filtering'] = True
        self.df.loc[self.df['filtering'] == False].to_csv(self.destpath + f'/dump_step_4_one_precursor_removed.csv')
        self.df.loc[(self.df['splitted_classes_names']==saveset[0]) & (self.df['filtering'] == False)].to_csv(self.destpath + f'/dump_step_4_one_precursor_{saveset[0]}_kept.csv')
    ...
    def split_train_test_by_id(self, split_ratio, id_column):
        ...
        in_test_set = self.df['crc_values'].apply(lambda id_: id_ < split_ratio * 2 ** 64)
        in_valid_set = self.df['crc_values'].apply(
            lambda id_: (split_ratio * 2 ** 64 <= id_) and (id_ < split_ratio * 2 * 2 ** 64))
        in_train_set = self.df['crc_values'].apply(lambda id_: id_ >= split_ratio * 2 * 2 ** 64)

        return self.df.loc[in_train_set], self.df.loc[in_valid_set], self.df.loc[in_test_set]
    ...
    def generate_training_files_random(self, split_ratio):
        ...
        sample = np.split( pd.Series(self.df['tokenized_products'].unique()).sample(frac=1, random_state=42),
                          [int((1.0 - split_ratio*2)*number_of_unique_products), int((1.0 - split_ratio)*number_of_unique_products)])
    ...
    # Generate the needed files for the classes
        ...
        with open(self.destpath + 'class-multi-train.txt', 'w') as f:
            classes_splitted = [(x.split('.')[0], x.rsplit('.',1)[0], x) for x in train.classes_ids.values]
        ...
        with open(self.destpath + 'class-multi-test.txt', 'w') as f:
            classes_splitted = [(x.split('.')[0], x.rsplit('.',1)[0], x) for x in test.classes_ids.values]
            incremental_classes = [' '.join(x) for x in classes_splitted]
            f.write('\n'.join(incremental_classes))
        ...
        with open(self.destpath + 'class-multi-valid.txt', 'w') as f:
            classes_splitted = [(x.split('.')[0], x.rsplit('.',1)[0], x) for x in valid.classes_ids.values]
            incremental_classes = [' '.join(x) for x in classes_splitted]
            f.write('\n'.join(incremental_classes))

...
def main(
    input_file_path: str, output_file_path: str, rxn_smiles_column: str, remove_atom_maps: bool,
    fragment_bond: str
):
    ...
    if remove_atom_maps:
        df[rxn_smiles_column] = df[rxn_smiles_column].apply(lambda x: remove_atom_mapping(x))
    df[rxn_smiles_column] = df[rxn_smiles_column].apply(
        lambda x: parse_extended_reaction_smiles(x, remove_atom_maps=False).

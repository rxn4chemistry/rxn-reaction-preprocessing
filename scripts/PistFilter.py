...
from rxn_reaction_preprocessing.reaction_filter import MixedReactionFilter
"""
...
class DataExtractionCleaning:
    ...
    def filter_dataset(self, filtername):
        ...
        self.df.loc[self.df['filtering'] == False
                    ].to_csv(self.destpath + f'/dump_step_2_{fname}_removed.csv')
    ...
    def tokenize(self):
        ...
        self.df['tokenized_precursors'] = [tokenize_smiles(x) for x in self.df.precursors]
    ...
    def do_set_on_precursors_and_products(self):
        ...
        self.df['precursors'] = [
            '.'.join(sorted(list(set(x.split('.'))))) for x in self.df['precursors_multi'].values
        ]
        self.df['products'] = [
            '.'.join(sorted(list(set(x.split('.'))))) for x in self.df['products_multi'].values
        ]
    ...
    def remove_residual_reagents(self):
        ...
        self.df['products_no_residual'] = [
            remove_res_reagents(x) for x in tqdm(self.df[['precursors', 'products']].values)
        ]
    ...
    def remove_single_precursor_rxns(self):
        ...
        self.df.loc[(self.df['splitted_classes_names'] == saveset[0])
                    & (self.df['filtering'] == False), 'filtering', ] = True
        self.df.loc[self.df['filtering'] == False
                    ].to_csv(self.destpath + f'/dump_step_4_one_precursor_removed.csv')
        self.df.loc[(self.df['splitted_classes_names'] == saveset[0])
    ...
    def generate_training_files_random(self, split_ratio):
        ...
        sample = np.split(
            pd.Series(self.df['tokenized_products'].unique()).sample(frac=1, random_state=42),
            [
    ...
    def generate_training_files(self, tipo: str = 'stable', split_ratio=0.05):
        ...
        for name, data in tqdm([('train', train), ('valid', valid), ('test', test)]):
            with open(self.destpath + f'precursors-{name}.txt', 'w') as f:
                f.write('\n'.join(data.tokenized_precursors.values))
            with open(self.destpath + f'product-{name}.txt', 'w') as f:
        ...
        with open(self.destpath + 'class-multi-train.txt', 'w') as f:
            classes_splitted = [
                (x.split('.')[0], x.rsplit('.', 1)[0], x) for x in train.classes_ids.values
            ]
        ...
        with open(self.destpath + 'class-multi-test.txt', 'w') as f:
            classes_splitted = [
                (x.split('.')[0], x.rsplit('.', 1)[0], x) for x in test.classes_ids.values
            ]
            incremental_classes = [' '.join(x) for x in classes_splitted]
            f.write('\n'.join(incremental_classes))
        ...
        with open(self.destpath + 'class-multi-valid.txt', 'w') as f:
            classes_splitted = [
                (x.split('.')[0], x.rsplit('.', 1)[0], x) for x in valid.classes_ids.values
            ]
            incremental_classes = [' '.join(x) for x in classes_splitted]
            f.write('\n'.join(incremental_classes))
